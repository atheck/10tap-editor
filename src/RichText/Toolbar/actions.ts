...
type ToolbarContextValue = (typeof ToolbarContext)[keyof typeof ToolbarContext];

interface ArgsToolbarCb {
	editor: EditorBridge;
	editorState: BridgeState;
	setToolbarContext: (ctx: ToolbarContextValue | ((prev: ToolbarContextValue) => ToolbarContextValue)) => void;
...
interface ToolbarItem {
	...
	image: ({ editor, editorState }: ArgsToolbarCb) => ImageSourcePropType;
}

const ToolbarItems = {
	...EditorActionType,
	// biome-ignore lint/style/useNamingConvention: toolbar item keys follow PascalCase convention
	ToggleH1: "toggle-h1",
	// biome-ignore lint/style/useNamingConvention: toolbar item keys follow PascalCase convention
	ToggleH2: "toggle-h2",
	// biome-ignore lint/style/useNamingConvention: toolbar item keys follow PascalCase convention
